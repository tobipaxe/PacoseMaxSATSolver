...
#ifdef LOGGING
      if (!runWholeProblem()) {
        std::cerr << "c Rename variables....: ERROR, UNDOING FAILED!" << std::endl;
        reset();
        exit(1);
      }
#endif // LOGGING
...
#ifdef LOGGING
    if (!runWholeProblem()) {
      std::cerr << "c   Shuffle clauses.....: ERROR, UNDOING FAILED!" << std::endl;
      reset();
      exit(1);
    }
#endif // LOGGING
  ...
  while (arraySize >= 1) {
    ...
    for (unsigned i = 1; i <= nbArrays; i++) {
      ...
      tmpWCNFFileName =
          folder + prefix + "_iteration_" +
          std::string(6 - std::min(6, (int)log10(fileCounter) + 1), '0') +
          std::to_string(fileCounter) + "_size-" + std::to_string(problemSize) +
          "_arraySize_" + std::to_string(pair.second - pair.first + 1) +
          "_arrayIter_" + std::to_string(i) + ".wcnf";
...
int main(int argc, char **argv) {
  ...
  // std::cout << "c random prefix.......: " << rndPrefix << std::endl;
  ...
  for (unsigned i = 1; i <= numberRounds; i++) {
    ...
    if (i > 1 && !rv)
      break;
    if (i != numberRounds) {
      if (shuffleClauses && clauses.size() > 1)
        ShuffleClauses();
      if (shuffleLiterals &&
  ...
  std::cout << "c mv " + lastTmpWCNFFileName + " " + reducedWcnfName
            << std::endl;
  if (!WIFEXITED(std::system(
          ("mv " + lastTmpWCNFFileName + " " + reducedWcnfName).c_str()))) {
